...
MM_ConfigurationIncrementalGenerational::createHeapWithManager(MM_EnvironmentBase *env, UDATA heapBytesRequested, MM_HeapRegionManager *regionManager)
{
...
#if defined(OMR_GC_VLHGC_CONCURRENT_COPY_FORWARD)
	// MM_HeapRegionManager *hrm = env->getExtensions()->heapRegionManager;
	...
	extensions->heapRegionStateTable =  MM_HeapRegionStateTable::newInstance(env->getForge(), heapBase, regionShift, regionCount);
